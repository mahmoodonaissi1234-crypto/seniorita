...
### Bulk actions (Items table)

Added in TICKET-121. Each row in the Items table has a checkbox (plus a "select all on this page" checkbox in the header); selecting any items shows a bulk action bar above the table with Activate, Deactivate, Move to category, and Delete. Selection is tracked by item id and survives paging, but is pruned whenever the filtered item list changes (e.g. an item is deleted elsewhere) so a stale id never sneaks into a bulk action. Activate/Deactivate/Move to category apply immediately; Delete shows the same "cannot be undone" confirmation modal used for single-item delete, sized to the selected count. All four actions are backed by `POST /api/items/bulk` (`{ ids, action, categoryId? }`), which re-checks each item exists before acting and logs one activity entry per item, same as the single-item routes. Any signed-in user can use bulk actions, matching the existing per-item edit/delete permissions.
